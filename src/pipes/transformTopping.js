...
const transformTopping = (toppingData, isSearchMode, searchKey) => {
    ...
    let uniqueToppingTypesName = removeDuplicateFromArr(toppingTypesName, 'ToppingTypeName');
    let finalSort = sortArrayAlphabatically(uniqueToppingTypesName, 'ToppingTypeId');
    ...
    finalSort.map((sorted) => {
        ...
        if (isSearchMode) {
            toppings = toppings.filter(
                createFilter(searchKey, KEYS_TO_FILTERS)
            );
        }
